...
async function crawl () {
  const browser = await puppeteer.launch({ headless: false, defaultViewport: { width: 1920, height: 800 }, devtools: true })
  ...
  while (true) {
    res = await page.evaluate((listItemIndex) => {
      const img = []
      let isEnd = false
      // while (listItemIndex > 0) {
      //   console.log(`listItemIndex: ${listItemIndex}`)
      //   console.log(document.querySelector(`#QuestionAnswers-answers .List div:nth-of-type(2) .List-item:nth-of-type(${listItemIndex})`))
      //   if (!document.querySelector(`#QuestionAnswers-answers .List div:nth-of-type(2) .List-item:nth-of-type(${listItemIndex})`)) {
      //     break
      //   }
      //   let i = 1
      //   while (i > 0) {
      //     const imgNoscriptDom = document.querySelector(`#QuestionAnswers-answers .List div:nth-of-type(2) .List-item:nth-of-type(${listItemIndex}) .RichText figure:nth-of-type(${i}) noscript`)
      //     if (!imgNoscriptDom) {
      //       break
      //     }
      //     const imgSrcMatch = imgNoscriptDom.innerText.match(/data-original="(\S*)"/) || imgNoscriptDom.innerText.match(/src="(\S*)"/)
    ...
    imgUrlAll = imgUrlAll.concat(res.imgUrls)
